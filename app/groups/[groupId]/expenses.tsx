...
export default function Expenses() {
  ...
  const load = useCallback(async () => {
    ...
    if (memberData) setMembers(memberData as unknown as GroupMember[]);
    if (expenseData) {
      setExpenses(expenseData as unknown as Expense[]);
      const expenseIds = expenseData.map((e) => e.id);
      if (expenseIds.length > 0) {
        const { data: splitData } = await supabase.from('expense_splits').select('*').in('expense_id', expenseIds);
        if (splitData) setSplits(splitData as ExpenseSplit[]);
      } else {
        setSplits([]);
      }
    }
  ...
  const balances = useMemo(() => {
    ...
    for (const e of expenses) paid[e.paid_by] = (paid[e.paid_by] ?? 0) + Number(e.amount);
    for (const s of splits) owed[s.user_id] = (owed[s.user_id] ?? 0) + Number(s.share);
  ...
  const addExpense = async () => {
    ...
    const shareRows = members.map((m) => ({
      ...
      user_id: m.user_id,
      share: m.user_id === session.user.id ? baseShare + remainder : baseShare,
    }));
    const { error: splitErr } = await supabase.from('expense_splits').insert(shareRows);
    setSaving(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <Card style={styles.form}>
        ...
        <Button label="Add expense" onPress={addExpense} loading={saving} style={{ backgroundColor: accentColor }} fullWidth />
      </Card>
    </View>
  );
}
