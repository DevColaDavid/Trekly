...
import { formatTimeInput, parseTimeInput } from '../lib/calendarMath';
...
export default function TimePickerGrid({ value, onChange }: Props) {
  ...
  const hours24 = safeValue.getHours();
  const minutes = safeValue.getMinutes();
  const isPM = hours24 >= 12;
  const hours12 = hours24 % 12 === 0 ? 12 : hours24 % 12;

  const [typedText, setTypedText] = useState(formatTimeInput(safeValue));
  ...
  useEffect(() => {
    ...
    setTypedText(formatTimeInput(value));
    setTypedError(false);
  }, [value]);

  const commitTyped = () => {
    const parsed = parseTimeInput(typedText, safeValue);
    if (!parsed) {
      setTypedError(true);
      return;
    }
  ...
  const bumpHour = (delta: number) => {
    ...
    onChange(withTime(safeValue, next, minutes));
  };
  const bumpMinute = (delta: number) => {
    ...
    onChange(withTime(safeValue, (hours24 + hourDelta + 24) % 24, next));
  };
  const togglePeriod = () => {
    const next = isPM ? hours24 - 12 : hours24 + 12;
    onChange(withTime(safeValue, (next + 24) % 24, minutes));
  };
...
const styles = StyleSheet.create({
  ...
  headerText: { color: 'white', fontSize: 15, fontWeight: '700', marginBottom: spacing.sm },
  typedInput: {
    color: 'white',
    fontSize: 13,
    fontWeight: '600',
